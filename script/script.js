...
// **** API CALL ****
let searchForecastForCity = (cityName = "Skopje") => {
  const API_KEY = "b1b69bc5f2f61003444a399cbbaa9c5d";
  // const API_URL = `https://api.openweathermap.org/data/2.5/forecast?q=CITY-NAME-HERE&units=metric&APPID=API-KEY-HERE`
...
let createHourlyRow = (reading) => {
  ...
  appendColumn(row, `${reading.weather[0].description}`);
  appendColumn(row, `${new Date(reading.dt * 1000)}`);
  appendColumn(row, `${reading.main.temp} C (${reading.main.feels_like} C)`);
  appendColumn(row, `${reading.main.humidity} %`);
  appendColumn(row, `${reading.wind.speed} km/h`);
...
let printHourlyTable = (measuredForeCastData) => {
  ...
  for (let reading of allReadings) {
    tableResult.appendChild(createHourlyRow(reading));
  }
};
